...
int GameView::displayWinSelect()
{
    ...
    string gameName[] = {"五子棋", "2048", "开心点点", "头脑风暴"};
...
BrainStorm::BrainStorm()
{
    ...
    gameView->setWindowsSize(25, 80);
    gameView->setWindowsName("头脑风暴");
}
void BrainStorm::location(HANDLE hOut, int x, int y) //定位
{
    COORD pos;
    pos.X = x;
    pos.Y = y;
...
void BrainStorm::prints(string s, int t) //按毫秒打印字符串
{
    int n = s.length();
    for (int i = 0; i < n; i++)
    {
...
void BrainStorm::printWINDOW() //设置窗口大小
{
...
void BrainStorm::Presskey(int m)
{
    char ch = _getch();
    if (ch == 27)
        exit(0);
...
int BrainStorm::GETMID(string s) //根据字符串长度获取居中位置
{
    int n = s.length() + 1;
    return (X - n) / 2;
}
void BrainStorm::sayHello()
{
...
void BrainStorm::question4()
{
    ...
    {
        string no1 = "  对不起，挑战失败！";
        location(hOut, GETMID(no1), Y / 2 - 1);
        prints(no1, 30);
        Sleep(1001);
        //location(hOut, GETMID("请按任意键继续. . ."), Y / 2 + 3);
    }//23 23 39 23 17 26 13 16 29 25 11 13 28
}
void BrainStorm::question3()
{
    system("cls");
...
bool BrainStorm::replayOrBreak()
{
    //system("cls");
    string replay = "按J回车 重新开始";
    location(hOut, GETMID(replay), Y / 2 + 3);
    cout << "                  ";
    location(hOut, GETMID(replay), Y / 2 + 3);
    cout << replay << endl;

    replay = "按K回车 回到菜单";
    location(hOut, GETMID(replay), Y / 2 + 4);
    cout << "                  ";
    location(hOut, GETMID(replay), Y / 2 + 4);
    cout << replay << endl;
